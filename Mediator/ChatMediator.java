		...
		if(participantes.containsKey(remetente) && participantes.containsKey(destinatario)) {
			...
			for(String proibido : palavrasProibidas) {
				if(mensagem.contains(proibido)) {
					pRemetente.recebeMensagem("Atenção", "Mensagem com palavras impróprias");
					return;
				}
			}
			//Se não há palavras proibidas na mensagem, então ela é enviada ao destinatario
